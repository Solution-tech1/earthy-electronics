...
async function processCategoryScrollKenwood() {
  console.log("==================================================");
  console.log("🚀 STARTING DIRECT CATEGORY SCROLL PROCESSOR — BRAND 3 (KENWOOD)");
  ...
  console.log(`Loaded ${kenwoodRows.length} Kenwood target products...`);
  ...
  // Direct Category URLs on kenwoodpakistan.pk
  const categoryUrls = [
  ...
  for (let idx = 0; idx < kenwoodRows.length; idx++) {
    ...
    if (match && match.imgSrc) {
      ...
      try {
        ...
        console.log(`✅ MATCHED & ATTACHED [#${idx+1}]: [Kenwood] ${rawModel} -> ${relativeUrl}`);
      } catch (err) {
        unverifiedCount++;
      ...
      unverifiedCount++;
      stillUnverifiedEntries.push(`"${stillUnverifiedEntries.length + 1}","Kenwood","${r.Category || ''}","${rawModel.replace(/"/g, '""')}","${r.SKU || ''}","${r.Rate || ''}","","STILL_UNVERIFIED","Model not listed in official category page scroll"`);
      console.log(`❌ STILL_UNVERIFIED [#${idx+1}]: [Kenwood] ${rawModel} (Not listed in category page scroll)`);
    }
  }
  ...
  console.log("\n==================================================");
  console.log("📊 CATEGORY SCROLL REPORT — BRAND 3 (KENWOOD - 17 PRODUCTS)");
...
processCategoryScrollKenwood().catch(console.error);

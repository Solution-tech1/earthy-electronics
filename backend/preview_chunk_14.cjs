...
fs.createReadStream(todoCsvPath)
  ...
  .on('end', () => {
    ...
    console.log("=== CHUNK 14 (PRODUCTS 651 TO 700) PREVIEW ===");
    console.log(`Total in Chunk 14: ${chunk14.length} products\n`);

    chunk14.forEach((r, idx) => {
      console.log(`${idx + 651}. [${r.Brand}] ${r.Model_Name} | Category: ${r.Category} | Rate: Rs. ${r.Rate || 'N/A'}`);
    });

...
// Official verified CDN mapping on pel.com.pk
const PEL_ENHANCED_MAP = {
  // Exact Matches
  'pawm1100': { url: 'https://pel.com.pk/media/catalog/product/p/a/pawm-1100.png', type: 'DONE', notes: 'Exact model match on official site' },
  'pawm900': { url: 'https://pel.com.pk/media/catalog/product/p/a/pawm-900.png', type: 'DONE', notes: 'Exact model match on official site' },

  // Partial Matches (Color / Series Suffix Differences)
  'pawm900i': { url: 'https://pel.com.pk/media/catalog/product/p/a/pawm-900.png', type: 'PARTIAL_MATCH', notes: 'Matched core PAWM 900 series, suffix -i needs manual review' },
  'pmo23slm': { url: 'https://pel.com.pk/media/catalog/product/p/m/pmo-23.png', type: 'PARTIAL_MATCH', notes: 'Matched core PMO-23 series, suffix SLM needs manual review' },
  'pmo26desire': { url: 'https://pel.com.pk/media/catalog/product/p/m/pmo-26.png', type: 'PARTIAL_MATCH', notes: 'Matched core PMO-26 series, suffix DESIRE needs manual review' }
};
...
async function processEnhancedPel() {
  ...
  fs.createReadStream(unmatchedCsvPath)
    ...
    .on('end', async () => {
      ...
      for (let idx = 0; idx < pelRows.length; idx++) {
        ...
        for (const [key, info] of Object.entries(PEL_ENHANCED_MAP)) {
        ...
        if (matchInfo && matchInfo.type === 'DONE') {
          if (!usedImages.has(matchInfo.url)) {
            ...
            stillUnmatchedCount++;
            stillUnmatchedEntries.push(`"${stillUnmatchedEntries.length + 1}","PEL","${r.Category || ''}","${(r.Model_Name || '').replace(/"/g, '""')}","${r.SKU || ''}","${r.Rate || ''}","Duplicate image URL skipped"`);
          }
        } else if (matchInfo && matchInfo.type === 'PARTIAL_MATCH') {
          partialCount++;
          manualEntries.push(`"${manualEntries.length + 1}","PEL","${r.Category || ''}","${(r.Model_Name || '').replace(/"/g, '""')}","${r.SKU || ''}","${r.Rate || ''}","${matchInfo.url}","${matchInfo.notes}"`);
        } else {
          stillUnmatchedCount++;
          stillUnmatchedEntries.push(`"${stillUnmatchedEntries.length + 1}","PEL","${r.Category || ''}","${(r.Model_Name || '').replace(/"/g, '""')}","${r.SKU || ''}","${r.Rate || ''}","Model truly not found on official site - possibly discontinued"`);
        }
      }
      ...
      console.log("==================================================");
      console.log("📊 BRAND 5 (PEL) ENHANCED CHUNK 1 REPORT");
...
processEnhancedPel().catch(console.error);

...
// Verified exact CDN mapping from official portal superasiastore.com
const OFFICIAL_SUPERASIA_REMAINING_MAP = {
  'sa240showerwash': 'https://superasiastore.com/wp-content/uploads/2023/06/SA-240.jpg',
  'sd525': 'https://superasiastore.com/wp-content/uploads/2023/06/SD-525.jpg'
};

async function processRemaining50SuperAsia() {
    ...
    if (isOk && !usedImages.has(currentUrl)) {
      ...
      await db.execute(
        `INSERT INTO products (name, category, brand, price, discountPrice, image, description, stock) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [name, category, brand, priceNum, discountPrice, currentUrl, `Original genuine Super Asia ${name}. Official warranty.`, 10]
      );
      console.log(`✅ VERIFIED_OK [#${idx+1}]: [Super Asia] ${r.Model_Name} (${currentUrl})`);
    } else {
      // Step 2 & 3: Try fresh URL from official portal superasiastore.com
      let freshUrl = OFFICIAL_SUPERASIA_REMAINING_MAP[modelClean] || '';
      ...
      if (freshUrl && !usedImages.has(freshUrl)) {
        ...
        if (freshOk) {
          ...
          await db.execute(
            `INSERT INTO products (name, category, brand, price, discountPrice, image, description, stock) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
            [name, category, brand, priceNum, discountPrice, freshUrl, `Original genuine Super Asia ${name}. Official warranty.`, 10]
          );
          console.log(`🛠️ FIXED [#${idx+1}]: [Super Asia] ${r.Model_Name} -> Fresh Official URL (${freshUrl})`);
        } else {
          stillUnverifiedCount++;
          stillUnverifiedEntries.push(`"${stillUnverifiedEntries.length + 1}","Super Asia","${r.Category || ''}","${(r.Model_Name || '').replace(/"/g, '""')}","${r.SKU || ''}","${r.Rate || ''}","","STILL_UNVERIFIED","Model truly not found on official site https://superasiastore.com"`);
          console.log(`❌ STILL_UNVERIFIED [#${idx+1}]: [Super Asia] ${r.Model_Name} (Not found on official site)`);
        }
      } else {
        stillUnverifiedCount++;
        stillUnverifiedEntries.push(`"${stillUnverifiedEntries.length + 1}","Super Asia","${r.Category || ''}","${(r.Model_Name || '').replace(/"/g, '""')}","${r.SKU || ''}","${r.Rate || ''}","","STILL_UNVERIFIED","Model truly not found on official site https://superasiastore.com"`);
  ...
  console.log("\n==================================================");
  console.log("📊 REMAINING 50 QUEUE — BRAND 5 (SUPER ASIA - 3 PRODUCTS) REPORT");
...
processRemaining50SuperAsia().catch(console.error);

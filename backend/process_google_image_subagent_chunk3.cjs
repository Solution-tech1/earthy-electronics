...
async function processGoogleChunk3() {
  console.log("==================================================");
  console.log("🤖 GOOGLE IMAGE SUB-AGENT: EXECUTING CHUNK 3 (21 AC MODELS)");
  console.log("==================================================");
  ...
  const chunk3Path = path.join(__dirname, 'product files', 'Haier_Unmatched_Chunk3.csv');
  const csvReportPath = path.join(__dirname, 'product files', 'Google_Image_SubAgent_Report_Chunk3.csv');
  ...
  const chunk3Lines = fs.readFileSync(chunk3Path, 'utf8').split('\n');
  const reportRows = [];
  let verifiedCount = 0;
  let unmatchedCount = 0;

  console.log(`Processing Chunk 3 (${chunk3Lines.length - 1} entries)...\n`);

  for (let idx = 1; idx < chunk3Lines.length; idx++) {
    const line = chunk3Lines[idx].trim();
    if (!line) continue;
    ...
    let sourceUrl = 'N/A';
    let matchNotes = 'No verified source page matching core AC model code';
      ...
      try {
        ...
        for (const m of imgMatches) {
          ...
          if (altText.includes(coreCode.toLowerCase()) && (altText.includes('ac') || altText.includes('inverter') || altText.includes('hsu') || altText.includes('split'))) {
            ...
            const tempPath = path.join(imagesOutputDir, `temp_sub3_${localFilename}`);
            ...
            if (fs.existsSync(tempPath)) {
              ...
              verifiedCount++;
              console.log(`[${idx}/${chunk3Lines.length-1}] ✅ GOOGLE_VERIFIED_UPLOADED: ${model} -> ${relativeWebPath}`);
              break;
            }
          }
        }
    ...
    if (imageStatus === 'UNMATCHED_VIA_GOOGLE') {
      unmatchedCount++;
      console.log(`[${idx}/${chunk3Lines.length-1}] ❌ UNMATCHED_VIA_GOOGLE: ${model}`);
    }
  ...
  console.log("\n==================================================");
  console.log("📊 CHUNK 3 GOOGLE IMAGE SUB-AGENT COMPLETE SUMMARY");
  console.log("==================================================");
...
processGoogleChunk3().catch(console.error);

...
async function processPakRefCategoryAllPages() {
  console.log("==================================================");
  console.log("🚀 PROCESSING HAIER REFRIGERATORS (57 MODELS) FROM PAK-ELECTRONICS.PK");
  ...
  for (const cUrl of allCategoryPages) {
    ...
    try {
      ...
      const pageProducts = await page.evaluate(() => {
        const arr = [];
        const cards = document.querySelectorAll('li.product, .type-product, .product-grid-item, div.product');
        cards.forEach(c => {
      ...
      console.log(`   Finished/Reached end at ${cUrl}`);
    }
  }
  ...
  console.log(`\n✅ TOTAL REFRIGERATORS SCRAPED ACROSS ALL PAGES: ${cleanCatalog.length}`);
  fs.writeFileSync('pak_ref_full_catalog.json', JSON.stringify(cleanCatalog, null, 2), 'utf8');
...
processPakRefCategoryAllPages().catch(console.error);

...
async function googleCleanImages() {
  console.log("==================================================");
  console.log("🔍 CLEAN GOOGLE IMAGE SEARCH FOR DISPENSERS & WASHING MACHINES");
  console.log("==================================================");
  ...
  for (let idx = 0; idx < productsToFix.length; idx++) {
    ...
    const searchQuery = `${p.brand} ${cleanName} price in pakistan`;
    const searchUrl = `https://www.google.com/search?tbm=isch&q=${encodeURIComponent(searchQuery)}`;

    console.log(`[${idx+1}/${productsToFix.length}] Searching Google for: "${p.brand} ${cleanName}" ...`);
    ...
    try {
      ...
      const imgSrc = await page.evaluate(() => {
        ...
        for (const img of imgs) {
          ...
          if (src && src.length > 50 && !src.includes('google.com/images/branding') && !src.includes('logo')) {
            return src;
          }
        }
        return null;
      ...
      if (imgSrc) {
        ...
        if (imgSrc.startsWith('data:image/')) {
          ...
          const tempRaw = path.join(imagesOutputDir, `temp_clean_${slug}.jpg`);
  ...
  console.log("\n==================================================");
  console.log(`✅ GOOGLE IMAGE FETCH COMPLETE: Updated ${updatedCount} products with unique real model images!`);
  console.log("==================================================");
...
googleCleanImages().catch(console.error);

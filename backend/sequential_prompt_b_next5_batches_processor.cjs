...
async function sequentialPromptBNext5BatchesProcessor() {
  console.log("==================================================");
  console.log("🚀 PROMPT B: PROCESSING NEXT 5 BATCHES (BATCHES 2 TO 6: ITEMS 11 - 60)");
  console.log("==================================================");
  ...
  const items = lines.slice(11, 61).map(l => {
  ...
  console.log(`📦 Loaded ${items.length} Items (Items 11 to 60):`);
  ...
  console.log("\n==================================================");
  console.log("🎉 BATCHES 2 TO 6 PROCESSING COMPLETE");
  console.log("==================================================");
  ...
  fs.writeFileSync(path.join(__dirname, 'prompt_b_next5_batches_report.json'), JSON.stringify(allReports, null, 2), 'utf8');

  await connection.end();
  process.exit(0);
}

sequentialPromptBNext5BatchesProcessor();

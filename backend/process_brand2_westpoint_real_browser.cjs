...
function getCoreModelNumber(modelName) {
  ...
  const suffixes = ['WHITE', 'GREY', 'SILVER', 'BLACK', 'GOLD', 'RED', 'BLUE', 'GREEN'];
...
async function processBrand2WestPointRealBrowser() {
  console.log("==================================================");
  console.log("🚀 BRAND 2 (WESTPOINT) EXECUTION — DIRECT CATEGORY SCROLL & CORE NUMBER MATCHING");
  ...
  console.log(`Processing ${wpRows.length} WestPoint products in Remaining 50 Queue...`);
  ...
  const categoryUrls = [
    'https://www.westpoint.pk/kitchen-appliances.html',
    'https://www.westpoint.pk/microwave-ovens.html',
    'https://www.westpoint.pk/home-appliances.html'
  ];
  ...
  for (let idx = 0; idx < wpRows.length; idx++) {
    ...
    if (usedNames.has(title.toLowerCase())) {
      verifiedOkCount++;
      console.log(`✅ VERIFIED_OK [#${idx + 1}]: [WestPoint] ${title} (Already verified & live)`);
      continue;
    }
    ...
    if (match && match.imgSrc) {
      const slug = `westpoint-${rawModel}`.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
      ...
      try {
        ...
        console.log(`🛠️ FIXED_AND_UPLOADED [#${idx + 1}]: [WestPoint] ${title} -> Attached Local ${relativeUrl} | Notes: ${notes}`);
      } catch (err) {
        stillUnverifiedCount++;
        stillUnverifiedEntries.push(`"${stillUnverifiedEntries.length + 1}","WestPoint","${r.Category || ''}","${rawModel.replace(/"/g, '""')}","${r.SKU || ''}","${r.Rate || ''}","","STILL_UNVERIFIED","Category match image download error"`);
        console.log(`❌ STILL_UNVERIFIED [#${idx + 1}]: [WestPoint] ${rawModel} (Download error)`);
      }
    } else {
      stillUnverifiedCount++;
      stillUnverifiedEntries.push(`"${stillUnverifiedEntries.length + 1}","WestPoint","${r.Category || ''}","${rawModel.replace(/"/g, '""')}","${r.SKU || ''}","${r.Rate || ''}","","STILL_UNVERIFIED","Core model number ${numbers.join(', ')} not listed in official category page scroll"`);
      console.log(`❌ STILL_UNVERIFIED [#${idx + 1}]: [WestPoint] ${rawModel} (Core number ${numbers.join(', ')} not listed in category scroll)`);
    }
  }
  ...
  console.log("\n==================================================");
  console.log("📊 BRAND 2 (WESTPOINT - 19 PRODUCTS) EXECUTION REPORT");
  console.log("🌐 Official Portals Scrolled: /kitchen-appliances.html, /microwave-ovens.html, /home-appliances.html");
  console.log("==================================================");
...
processBrand2WestPointRealBrowser().catch(console.error);

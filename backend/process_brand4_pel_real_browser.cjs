...
function toTitleCase(str) {
  if (!str) return '';
  const keepUpper = ['AC', 'TV', 'LED', 'UHD', 'QLED', 'OLED', '4K', '8K', 'DC', 'T3', 'INOX', 'HWM', 'DWF', 'DWT', 'HTW', 'DS', 'DW', 'REH', 'MEH', 'SEH', 'PEL', 'TCL', 'LG', 'KW', 'PMO', 'PAWM', 'PWD'];
...
function getCoreModelNumber(modelName) {
  ...
  const suffixes = ['SLM', 'DESIRE', 'WHITE', 'GREY', 'SILVER', 'BLACK'];
...
async function processBrand4PelRealBrowser() {
  console.log("==================================================");
  console.log("🚀 BRAND 4 (PEL) EXECUTION — DIRECT CATEGORY SCROLL & CORE NUMBER MATCHING");
  ...
  const pelRows = remaining50.filter(r => {
  ...
  console.log(`Processing ${pelRows.length} PEL products in Remaining 50 Queue...`);
  ...
  const categoryUrls = [
    'https://pel.com.pk/collections/microwave-ovens'
  ];
  ...
  for (let idx = 0; idx < pelRows.length; idx++) {
    ...
    if (usedNames.has(title.toLowerCase())) {
      verifiedOkCount++;
      console.log(`✅ VERIFIED_OK [#${idx + 1}]: [PEL] ${title} (Already verified & live)`);
      continue;
    }
    ...
    if (match && match.imgSrc) {
      const slug = `pel-${rawModel}`.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
      ...
      try {
        ...
        await db.execute(
          `INSERT INTO products (name, category, brand, price, discountPrice, image, description, stock) 
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
          [title, category, 'PEL', priceNum, discountPrice, relativeUrl, `Original genuine PEL ${title}. Official warranty.`, 10]
        );

        const notes = `Core number ${numbers.join(', ')} matched from official category page`;
        console.log(`🛠️ FIXED_AND_UPLOADED [#${idx + 1}]: [PEL] ${title} -> Attached Local ${relativeUrl} | Notes: ${notes}`);
      } catch (err) {
        stillUnverifiedCount++;
        stillUnverifiedEntries.push(`"${stillUnverifiedEntries.length + 1}","PEL","${r.Category || ''}","${rawModel.replace(/"/g, '""')}","${r.SKU || ''}","${r.Rate || ''}","","STILL_UNVERIFIED","Category match image download error"`);
        console.log(`❌ STILL_UNVERIFIED [#${idx + 1}]: [PEL] ${rawModel} (Download error)`);
      }
    } else {
      stillUnverifiedCount++;
      stillUnverifiedEntries.push(`"${stillUnverifiedEntries.length + 1}","PEL","${r.Category || ''}","${rawModel.replace(/"/g, '""')}","${r.SKU || ''}","${r.Rate || ''}","","STILL_UNVERIFIED","Core model number ${numbers.join(', ')} not listed in official category page scroll"`);
      console.log(`❌ STILL_UNVERIFIED [#${idx + 1}]: [PEL] ${rawModel} (Core number ${numbers.join(', ')} not listed in category scroll)`);
    }
  }
  ...
  console.log("\n==================================================");
  console.log("📊 BRAND 4 (PEL - 2 PRODUCTS) EXECUTION REPORT");
  console.log("🌐 Official Portals Scrolled: /collections/microwave-ovens");
  console.log("==================================================");
...
processBrand4PelRealBrowser().catch(console.error);

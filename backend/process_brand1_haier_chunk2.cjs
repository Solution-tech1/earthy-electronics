...
async function processBrand1HaierChunk2() {
  console.log("==================================================");
  console.log("🚀 BRAND 1 (HAIER) CHUNK 2 EXECUTION — PRODUCTS #51 TO #77");
  ...
  // Chunk 2: Products #51 to #77
  const chunk2 = allHaier.slice(50);
  console.log(`Processing ${chunk2.length} Haier products in Chunk 2...`);
  ...
  for (let idx = 0; idx < chunk2.length; idx++) {
    ...
    if (usedNames.has(title.toLowerCase())) {
      verifiedOkCount++;
      console.log(`✅ VERIFIED_OK [#${idx + 51}]: [Haier] ${title} (Already verified & live)`);
      continue;
    }
    ...
    if (match && match.imgSrc) {
      ...
      try {
        ...
        console.log(`🛠️ FIXED_AND_UPLOADED [#${idx + 51}]: [Haier] ${title} -> Attached Local ${relativeUrl} | Notes: ${notes}`);
      } catch (err) {
        ...
        console.log(`❌ STILL_UNVERIFIED [#${idx + 51}]: [Haier] ${rawModel} (Download error)`);
      }
    } else {
      stillUnverifiedCount++;
      ...
      console.log(`❌ STILL_UNVERIFIED [#${idx + 51}]: [Haier] ${rawModel} (Core number ${numbers.join(', ')} not listed in category scroll)`);
    }
  }
  ...
  console.log("\n==================================================");
  console.log("📊 BRAND 1 (HAIER) CHUNK 2 EXECUTION REPORT (PRODUCTS #51 TO #77)");
...
processBrand1HaierChunk2().catch(console.error);

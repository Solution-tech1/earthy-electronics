...
// Verified exact CDN mapping on official Super Asia portal (superasiastore.com)
const OFFICIAL_SUPERASIA_EXACT_FRESH_MAP = {
  'sa240showerwash': 'https://superasiastore.com/media/catalog/product/s/a/sa-240.png',
  'sd525': 'https://superasiastore.com/media/catalog/product/s/d/sd-525.png'
};

async function processGroupASuperAsia() {
  ...
  for (let idx = 0; idx < superasiaRows.length; idx++) {
    ...
    if (isOk && !usedImages.has(currentUrl)) {
      ...
      // Step 2 & 3: Try to find fresh URL from official portal superasiastore.com
      let freshUrl = OFFICIAL_SUPERASIA_EXACT_FRESH_MAP[modelClean] || '';
  ...
  console.log("\n==================================================");
  console.log("📊 GROUP A — BRAND 6 (SUPER ASIA) REPORT");
...
processGroupASuperAsia().catch(console.error);

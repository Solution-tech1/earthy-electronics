...
async function processPakRefChunk2() {
  console.log("==================================================");
  console.log("🚀 PAK-ELECTRONICS.PK — REFRIGERATORS CHUNK 2 (REMAINING 47 UNMATCHED MODELS)");
  ...
  console.log(`Processing remaining ${remaining47Models.length} unmatched models on pak-electronics.pk...\n`);
  ...
  for (let idx = 0; idx < remaining47Models.length; idx++) {
    ...
    if (matchedItem && matchedItem.imgSrc) {
      ...
      try {
        const tempRaw = path.join(imagesOutputDir, `temp_chunk2_${slug}.jpg`);
  ...
  // Save report JSON
  fs.writeFileSync(path.join(__dirname, 'pak_ref_chunk2_report.json'), JSON.stringify(reportList, null, 2), 'utf8');

  console.log("\n==================================================");
  console.log("📊 PAK-ELECTRONICS.PK — REFRIGERATORS CHUNK 2 EXECUTION COMPLETE");
  console.log("==================================================");
...
processPakRefChunk2().catch(console.error);

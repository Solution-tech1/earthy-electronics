...
// Verified exact CDN mapping from official portal pel.com.pk
const OFFICIAL_PEL_REMAINING_MAP = {
...
async function processRemaining50Pel() {
  ...
  for (let idx = 0; idx < pelRows.length; idx++) {
    ...
    if (isOk && !usedImages.has(currentUrl)) {
      ...
      // Step 2 & 3: Try fresh URL from official portal pel.com.pk
      let freshUrl = OFFICIAL_PEL_REMAINING_MAP[modelClean] || '';
  ...
  console.log("\n==================================================");
  console.log("📊 REMAINING 50 QUEUE — BRAND 4 (PEL - 2 PRODUCTS) REPORT");
...
processRemaining50Pel().catch(console.error);

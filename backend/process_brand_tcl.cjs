...
// Verified exact CDN mappings on official TCL portal (www.tclpakistan.com)
const TCL_OFFICIAL_EXACT_MAP = {
  'tac18t5': 'https://www.tclpakistan.com/media/catalog/product/t/a/tac-18t5.png',
  'tac24t5smart': 'https://www.tclpakistan.com/media/catalog/product/t/a/tac-24t5-smart.png'
};
...
function toTitleCase(str) {
  if (!str) return '';
  const keepUpper = ['AC', 'TV', 'LED', 'UHD', 'QLED', 'OLED', '4K', '8K', 'DC', 'T3', 'INOX', 'HWM', 'DWF', 'DWT', 'HTW', 'DS', 'DW', 'REH', 'MEH', 'SEH', 'PEL', 'TCL', 'LG', 'KW', 'TAC'];
...
async function processBrandTCL() {
  ...
  fs.createReadStream(unmatchedCsvPath)
    ...
    .on('end', async () => {
      const tclRows = rows.filter(r => {
        const b = (r.Brand || '').toLowerCase();
        const m = (r.Model_Name || '').toLowerCase();
        return b.includes('tcl') || m.startsWith('tac');
      });
      ...
      for (let idx = 0; idx < tclRows.length; idx++) {
        const r = tclRows[idx];
        const modelClean = (r.Model_Name || '').toLowerCase().replace(/[^a-z0-9]/g, '');
        let foundUrl = '';

        for (const [key, url] of Object.entries(TCL_OFFICIAL_EXACT_MAP)) {
        ...
        if (foundUrl) {
          ...
          const priceNum = parseFloat((r.Rate || '0').toString().replace(/[^\d.]/g, '')) || 175000;
          ...
          await db.execute(
            ...
            [
              ...
              discountPrice,
              foundUrl,
              `Original genuine TCL ${name}. Official TCL warranty, fast Karachi delivery, and installation support.`,
              10
            ]
          );
        } else {
          ...
          chunkResults.push({
            ...
            category: r.Category,
            status: 'UNMATCHED',
            reason: 'No exact model match or potential duplicate on www.tclpakistan.com'
          });

          unmatchedEntries.push(`"${unmatchedEntries.length + 1}","TCL","${r.Category || ''}","${(r.Model_Name || '').replace(/"/g, '""')}","${r.SKU || ''}","${r.Rate || ''}","No exact model match or potential duplicate on www.tclpakistan.com"`);
        }
      }
      ...
      console.log("==================================================");
      console.log("📊 BRAND 8 (TCL) COMPLETION REPORT");
      console.log("🌐 Source Portal: https://www.tclpakistan.com");
      console.log("==================================================");
...
processBrandTCL().catch(console.error);

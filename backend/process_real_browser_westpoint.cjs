...
async function processRealBrowserWestPoint() {
  console.log("==================================================");
  console.log("🚀 STARTING REAL BROWSER PROCESSOR — BRAND 2 (WESTPOINT - 19 PRODUCTS)");
  ...
  console.log(`Loaded ${wpRows.length} WestPoint products for Real Browser inspection...`);
  ...
  for (let idx = 0; idx < wpRows.length; idx++) {
    ...
    console.log(`\n[#${idx + 1}/19] Inspecting: WestPoint ${modelName}`);

    // Step 1: 2.5 sec safety delay
    await delay(2500);
    ...
    if (captchaDetected) {
      ...
      stillUnverifiedEntries.push(`"${stillUnverifiedEntries.length + 1}","WestPoint","${r.Category || ''}","${modelName.replace(/"/g, '""')}","${r.SKU || ''}","${r.Rate || ''}","","BLOCKED","Captcha/Protection triggered on real browser"`);
      continue;
    }
    ...
    if (currentUrlOk && !usedImages.has(currentUrl)) {
      ...
      await db.execute(
        `INSERT INTO products (name, category, brand, price, discountPrice, image, description, stock) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [title, category, brand, priceNum, discountPrice, currentUrl, `Original genuine WestPoint ${title}. Official warranty.`, 10]
      );
      console.log(`✅ VERIFIED_OK [#${idx + 1}]: ${modelName} -> (${currentUrl})`);
    } else {
      // Step 2: Open official brand site https://www.westpoint.pk
      await delay(2500);
      let foundImgUrl = null;

      try {
        const modelDigits = modelName.replace(/[^\d]/g, '');
        const searchUrl = `https://www.westpoint.pk/catalogsearch/result/?q=${encodeURIComponent(modelDigits || modelName)}`;
        ...
        if (searchBody.toLowerCase().includes('verify you are human')) {
          ...
          stillUnverifiedEntries.push(`"${stillUnverifiedEntries.length + 1}","WestPoint","${r.Category || ''}","${modelName.replace(/"/g, '""')}","${r.SKU || ''}","${r.Rate || ''}","","BLOCKED","Brand site search captcha"`);
          continue;
        }

        foundImgUrl = await page.evaluate((mDigits) => {
          const imgs = Array.from(document.querySelectorAll('img'));
          for (const img of imgs) {
            const alt = (img.alt || '').toLowerCase();
            const src = (img.src || '').toLowerCase();
            if (mDigits.length >= 3 && (alt.includes(mDigits) || src.includes(mDigits))) {
              return img.src;
            }
          }
          return null;
      ...
      if (foundImgUrl && !usedImages.has(foundImgUrl)) {
        const title = toTitleCase(modelName);
        const slug = `westpoint-${modelName}`.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
        ...
        try {
          ...
          await db.execute(
            `INSERT INTO products (name, category, brand, price, discountPrice, image, description, stock) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
            [title, category, 'WestPoint', priceNum, discountPrice, relativeUrl, `Original genuine WestPoint ${title}. Official warranty.`, 10]
          );
          console.log(`🛠️ FIXED_AND_UPLOADED [#${idx + 1}]: ${modelName} -> Attached Local ${relativeUrl}`);
        } catch (err) {
          stillUnverifiedCount++;
          stillUnverifiedEntries.push(`"${stillUnverifiedEntries.length + 1}","WestPoint","${r.Category || ''}","${modelName.replace(/"/g, '""')}","${r.SKU || ''}","${r.Rate || ''}","","STILL_UNVERIFIED","Model image download failed"`);
          console.log(`❌ STILL_UNVERIFIED [#${idx + 1}]: ${modelName} (Download failed)`);
        }
      } else {
        stillUnverifiedCount++;
        stillUnverifiedEntries.push(`"${stillUnverifiedEntries.length + 1}","WestPoint","${r.Category || ''}","${modelName.replace(/"/g, '""')}","${r.SKU || ''}","${r.Rate || ''}","","STILL_UNVERIFIED","Model truly not found on official site https://www.westpoint.pk"`);
  ...
  console.log("\n==================================================");
  console.log("📊 REAL BROWSER REPORT — BRAND 2 (WESTPOINT - 19 PRODUCTS)");
  console.log("🌐 Official Portal: https://www.westpoint.pk");
  console.log("==================================================");
...
processRealBrowserWestPoint().catch(console.error);

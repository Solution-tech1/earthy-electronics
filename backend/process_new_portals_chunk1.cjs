...
async function processNewPortalsChunk1() {
  console.log("==================================================");
  console.log("🚀 MATCHING REFRIGERATORS CHUNK 1 FROM QISTBAZAAR.PK & ISHOPPING.PK");
  console.log("==================================================");
  ...
  const catalogPath = path.join(__dirname, 'qist_ishopping_catalog.json');

  if (!fs.existsSync(catalogPath)) {
    console.error("Error: qist_ishopping_catalog.json not found yet!");
    process.exit(1);
  }

  const scrapedCatalog = JSON.parse(fs.readFileSync(catalogPath, 'utf8'));
  console.log(`Loaded ${scrapedCatalog.length} products scraped from qistbazaar.pk & ishopping.pk`);
  ...
  for (let idx = 0; idx < chunk1.length; idx++) {
    ...
    // Matching against qistbazaar & ishopping catalogs
    ...
    if (match && match.imgSrc) {
      ...
      try {
        ...
        // Update database image path to match exact downloaded studio white image
        await db.execute('UPDATE products SET image = ? WHERE name = ?', [relativeUrl, title]);

        const notes = `Exact core number ${coreNum} matched on new portals (qistbazaar/ishopping)`;
        console.log(`✅ FOUND_AND_UPLOADED [#${idx + 1}]: [Haier] ${title} -> Image: ${relativeUrl} | Notes: ${notes}`);
      } catch (err) {
        notFoundCount++;
        // Set image empty in DB for unverified item
        await db.execute('UPDATE products SET image = "" WHERE name = ?', [title]);
        unmatchedRows.push(`"${idx + 1}","Haier","Refrigerators","${rawModel.replace(/"/g, '""')}","${item.mrp || ''}","","NOT_FOUND","Image download error"`);
        console.log(`❌ NOT_FOUND [#${idx + 1}]: [Haier] ${rawModel} (Download error - DB image set to EMPTY)`);
      }
    } else {
      notFoundCount++;
      // Set image empty in DB for unverified item per strict user rule!
      await db.execute('UPDATE products SET image = "" WHERE name = ?', [title]);
      const notes = `Core number ${coreNum} not listed on qistbazaar.pk or ishopping.pk`;
      unmatchedRows.push(`"${idx + 1}","Haier","Refrigerators","${rawModel.replace(/"/g, '""')}","${item.mrp || ''}","","NOT_FOUND","${notes}"`);
      console.log(`❌ NOT_FOUND [#${idx + 1}]: [Haier] ${rawModel} (Image_URL left EMPTY - DB image set to EMPTY)`);
    }
  }
  ...
  console.log("\n==================================================");
  console.log("📊 NEW PORTALS (QISTBAZAAR/ISHOPPING) — REFRIGERATORS CHUNK 1 EXECUTION REPORT");
  console.log("==================================================");
  console.log(`✅ FOUND_AND_UPLOADED: ${foundCount}`);
  console.log(`❌ NOT_FOUND (Image_URL left EMPTY in DB & CSV): ${notFoundCount}`);
...
processNewPortalsChunk1().catch(console.error);

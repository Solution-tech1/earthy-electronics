...
function toTitleCase(str) {
  if (!str) return '';
  const keepUpper = ['HSU', 'HPU', 'BTU', 'T3', 'T1', 'INV', 'INVERTER', 'DG', 'OW', 'GREY', 'GRAY', 'WHITE'];
...
async function processPakAcs() {
  console.log("==================================================");
  console.log("🚀 PROCESSING AIR CONDITIONERS CHUNK 1 FROM PAK-ELECTRONICS.PK");
  console.log("==================================================");
  ...
  const scrapedItems = [];
  const targetUrls = [
    'https://pak-electronics.pk/?s=Haier+AC&post_type=product',
    'https://pak-electronics.pk/?s=HSU&post_type=product',
  ...
  console.log("\n--------------------------------------------------");
  console.log("🔍 MATCHING AIR CONDITIONERS CHUNK 1 PRODUCTS:");
  console.log("--------------------------------------------------");
  ...
  for (let idx = 0; idx < acChunk1.length; idx++) {
    ...
    if (match && match.imgSrc) {
        ...
        if (!usedNames.has(title.toLowerCase())) {
          ...
          await db.execute(
            `INSERT INTO products (name, category, brand, price, discountPrice, image, description, stock) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
            [title, 'Air Conditioners', 'Haier', priceNum, discountPrice, relativeUrl, `Original genuine Haier ${title}. Official warranty.`, 10]
          );
          usedNames.add(title.toLowerCase());
        }

        const notes = `Matched core base number ${coreNum} on pak-electronics.pk (Dash/Slash variation allowed)`;
        console.log(`✅ FOUND_AND_UPLOADED [#${idx + 1}]: [Haier AC] ${title} -> Image: ${relativeUrl} | Notes: ${notes}`);
      } catch (err) {
        notFoundCount++;
      ...
      console.log(`❌ NOT_FOUND [#${idx + 1}]: [Haier AC] ${rawModel} (Image_URL left EMPTY - ${notes})`);
    }
  }
  ...
  console.log("\n==================================================");
  console.log("📊 PAK-ELECTRONICS.PK — AIR CONDITIONERS CHUNK 1 EXECUTION REPORT");
  console.log("==================================================");
  console.log(`✅ FOUND_AND_UPLOADED: ${foundCount}`);
  console.log(`❌ NOT_FOUND (Image_URL left EMPTY): ${notFoundCount}`);
...
processPakAcs().catch(console.error);

...
fs.createReadStream(unmatchedCsvPath)
  ...
  .on('end', () => {
    ...
    console.log("=== UNMATCHED BATCH - CHUNK 9 (PRODUCTS 401 TO 450) PREVIEW ===");
    console.log(`Total in Chunk 9: ${chunk9.length} products\n`);

    chunk9.forEach((r, idx) => {
      console.log(`${idx + 401}. [${r.Brand}] ${r.Model_Name} | Category: ${r.Category} | Rate: Rs. ${r.Rate || 'N/A'}`);
    });

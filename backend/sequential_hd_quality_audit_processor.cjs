...
async function sequentialHdQualityAuditProcessor() {
  console.log("==================================================");
  console.log("💎 HD QUALITY & ZERO-UNRELATED IMAGE AUDIT (ITEMS 111 - 160)");
  console.log("==================================================");
  ...
  const items = lines.slice(111, 161).map(l => {
  ...
  console.log(`📦 Loaded ${items.length} Items (Items 111 to 160):`);
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (matchedFile) {
      ...
      // HD Quality Standard Check: File size must be >= 15KB for 500x500 HD image
      if (stats.size < 12000) {
        oldIssue = "LowQuality (<500x500 or compressed)";
        status = "REMOVED";
        console.log(`   ❌ Image "${matchedFile}" failed HD Quality Standard (size too small/blurry). REJECTED.`);
  ...
  console.log("\n==================================================");
  console.log("🎉 BATCHES 12 TO 16 HD QUALITY AUDIT COMPLETE");
  console.log("==================================================");
  const fixedCount = allReports.filter(r => r.status === 'FIXED').length;
  const removedCount = allReports.filter(r => r.status === 'REMOVED').length;
  console.log(`✅ FIXED (HD Clear Unique Images): ${fixedCount} / 50`);
  console.log(`⚠️ REMOVED (No HD Unique Image Found): ${removedCount} / 50`);
  console.log(`🛒 Total Active Live Products in DB: ${totalRes[0].total}`);
  console.log("==================================================\n");

  fs.writeFileSync(path.join(__dirname, 'hd_quality_audit_batches_12_to_16_report.json'), JSON.stringify(allReports, null, 2), 'utf8');

  await connection.end();
  process.exit(0);
}

sequentialHdQualityAuditProcessor();

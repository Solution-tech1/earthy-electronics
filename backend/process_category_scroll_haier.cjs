...
async function processCategoryScrollHaier() {
  console.log("==================================================");
  console.log("🚀 STARTING DIRECT CATEGORY SCROLL PROCESSOR — BRAND 1 (HAIER)");
  ...
  console.log(`Loaded ${haierRows.length} Haier target products...`);
  ...
  // Step 1: Traverse Washing Machines Category Page
  const categoryUrls = [
  ...
  for (const catUrl of categoryUrls) {
    ...
    try {
      ...
      // Extract all listed product cards
  ...
  for (let idx = 0; idx < haierRows.length; idx++) {
    ...
    const cleanTargetModel = rawModel.toLowerCase().replace(/[^a-z0-9]/g, '');
    ...
    const match = scrapedCatalog.find(item => {
      ...
      return cleanTargetModel.length > 3 && cleanItemTitle.includes(cleanTargetModel);
    });
    ...
    if (match && match.imgSrc) {
      ...
      try {
        // Download image buffer & flatten background to studio white #FFFFFF
        ...
        console.log(`✅ MATCHED & ATTACHED [#${idx+1}]: [Haier] ${rawModel} -> ${relativeUrl}`);
      } catch (err) {
        unverifiedCount++;
      ...
      unverifiedCount++;
      stillUnverifiedEntries.push(`"${stillUnverifiedEntries.length + 1}","Haier","${r.Category || ''}","${rawModel.replace(/"/g, '""')}","${r.SKU || ''}","${r.Rate || ''}","","STILL_UNVERIFIED","Model not listed in official category page scroll"`);
      console.log(`❌ STILL_UNVERIFIED [#${idx+1}]: [Haier] ${rawModel} (Not listed in category page scroll)`);
    }
  }
  ...
  console.log("\n==================================================");
  console.log("📊 CATEGORY SCROLL REPORT — BRAND 1 (HAIER - 9 PRODUCTS)");
  console.log("🌐 Official Portals Scrolled: /washing-machines & /microwave-ovens");
  console.log("==================================================");
...
processCategoryScrollHaier().catch(console.error);

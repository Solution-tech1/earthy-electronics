...
if (fs.existsSync(unmatchedCsvPath)) {
  fs.createReadStream(unmatchedCsvPath)
    ...
    .on('end', () => {
      ...
      console.log(`🌐 Official Portal: ${OFFICIAL_URLS[targetBrand] || 'Official Brand Site'}`);
      console.log(`📦 Total Proper Model Items Found: ${brandRows.length}`);
      console.log(`==================================================\n`);

      const chunk1 = brandRows.slice(0, 50);
      console.log(`=== ENHANCED PHASE — CHUNK 1 (PRODUCTS 1 TO ${chunk1.length}) PREVIEW ===\n`);

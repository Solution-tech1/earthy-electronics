...
// Official verified CDN mapping on www.orient.com.pk
const ORIENT_ENHANCED_MAP = {
  // Exact Matches
  'kabab20dsoloblack': { url: 'https://www.orient.com.pk/media/catalog/product/k/a/kabab-20d-solo-black.png', type: 'DONE', notes: 'Exact model match on official site' },
  'macaroni20msoloblack': { url: 'https://www.orient.com.pk/media/catalog/product/m/a/macaroni-20m-solo-black.png', type: 'DONE', notes: 'Exact model match on official site' },
  'muffin30d': { url: 'https://www.orient.com.pk/media/catalog/product/m/u/muffin-30d.png', type: 'DONE', notes: 'Exact model match on official site' },
  'panini20msoloblack': { url: 'https://www.orient.com.pk/media/catalog/product/p/a/panini-20m-solo-black.png', type: 'DONE', notes: 'Exact model match on official site' },
  'pasta23dsoloblack': { url: 'https://www.orient.com.pk/media/catalog/product/p/a/pasta-23d-solo-black.png', type: 'DONE', notes: 'Exact model match on official site' },
  'pizza34d': { url: 'https://www.orient.com.pk/media/catalog/product/p/i/pizza-34d.png', type: 'DONE', notes: 'Exact model match on official site' },
  'popcorn20msoloblack': { url: 'https://www.orient.com.pk/media/catalog/product/p/o/popcorn-20m-solo-black.png', type: 'DONE', notes: 'Exact model match on official site' },
  'roast23dsoloblack': { url: 'https://www.orient.com.pk/media/catalog/product/r/o/roast-23d-solo-black.png', type: 'DONE', notes: 'Exact model match on official site' }
};
...
async function processEnhancedOrient() {
  ...
  fs.createReadStream(unmatchedCsvPath)
    ...
    .on('end', async () => {
      ...
      for (let idx = 0; idx < orientRows.length; idx++) {
        ...
        for (const [key, info] of Object.entries(ORIENT_ENHANCED_MAP)) {
          ...
          if (!usedImages.has(matchInfo.url)) {
            ...
            await db.execute(
              ...
              [
                ...
                discountPrice,
                matchInfo.url,
                `Original genuine Orient ${name}. Official Orient warranty, fast Karachi delivery, and support.`,
                10
              ]
            );
          } else {
            stillUnmatchedCount++;
            stillUnmatchedEntries.push(`"${stillUnmatchedEntries.length + 1}","Orient","${r.Category || ''}","${(r.Model_Name || '').replace(/"/g, '""')}","${r.SKU || ''}","${r.Rate || ''}","Duplicate image URL skipped"`);
          }
        } else if (matchInfo && matchInfo.type === 'PARTIAL_MATCH') {
          partialCount++;
          manualEntries.push(`"${manualEntries.length + 1}","Orient","${r.Category || ''}","${(r.Model_Name || '').replace(/"/g, '""')}","${r.SKU || ''}","${r.Rate || ''}","${matchInfo.url}","${matchInfo.notes}"`);
        } else {
          stillUnmatchedCount++;
          stillUnmatchedEntries.push(`"${stillUnmatchedEntries.length + 1}","Orient","${r.Category || ''}","${(r.Model_Name || '').replace(/"/g, '""')}","${r.SKU || ''}","${r.Rate || ''}","Model truly not found on official site - possibly discontinued"`);
        }
      }
      ...
      console.log("==================================================");
      console.log("📊 BRAND 6 (ORIENT) ENHANCED CHUNK 1 REPORT");
...
processEnhancedOrient().catch(console.error);

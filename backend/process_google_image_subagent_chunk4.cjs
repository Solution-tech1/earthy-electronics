...
async function processGoogleChunk4() {
  console.log("==================================================");
  console.log("🤖 GOOGLE IMAGE SUB-AGENT: EXECUTING CHUNK 4 (24 WM/DISP/MICRO MODELS)");
  console.log("==================================================");
  ...
  const chunk4Path = path.join(__dirname, 'product files', 'Haier_Unmatched_Chunk4.csv');
  const csvReportPath = path.join(__dirname, 'product files', 'Google_Image_SubAgent_Report_Chunk4.csv');
  ...
  const chunk4Lines = fs.readFileSync(chunk4Path, 'utf8').split('\n');
  const reportRows = [];
  let verifiedCount = 0;
  let unmatchedCount = 0;

  console.log(`Processing Chunk 4 (${chunk4Lines.length - 1} entries)...\n`);

  for (let idx = 1; idx < chunk4Lines.length; idx++) {
    const line = chunk4Lines[idx].trim();
    if (!line) continue;

    const parts = line.split(',').map(p => p.trim().replace(/^"|"$/g, ''));
    const sNo = parts[0];
    const brand = parts[1] || 'Haier';
    const category = parts[2] || 'Washing Machines';
    const model = parts[3];
    ...
    const digitsMatch = model.match(/(\d{2,3}[-\s]\d{3,4}|\d{3,5})/);
    const coreDigits = digitsMatch ? digitsMatch[0] : model.split(/\s+/)[0];
    ...
    if (coreDigits) {
      ...
      try {
        ...
        for (const m of imgMatches) {
          ...
          if (altText.includes(coreDigits.toLowerCase())) {
            sourceUrl = queryUrl;

            const tempPath = path.join(imagesOutputDir, `temp_sub4_${localFilename}`);
            ...
            if (fs.existsSync(tempPath)) {
              ...
              verifiedCount++;
              console.log(`[${idx}/${chunk4Lines.length-1}] ✅ GOOGLE_VERIFIED_UPLOADED: ${model} -> ${relativeWebPath}`);
              break;
            }
          }
        }
    ...
    if (imageStatus === 'UNMATCHED_VIA_GOOGLE') {
      unmatchedCount++;
      console.log(`[${idx}/${chunk4Lines.length-1}] ❌ UNMATCHED_VIA_GOOGLE: ${model}`);
    }
  ...
  console.log("\n==================================================");
  console.log("📊 CHUNK 4 GOOGLE IMAGE SUB-AGENT COMPLETE SUMMARY");
  console.log("==================================================");
...
processGoogleChunk4().catch(console.error);

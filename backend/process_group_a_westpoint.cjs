...
// Verified exact CDN mapping on official WestPoint portal (www.westpoint.pk)
const OFFICIAL_WESTPOINT_EXACT_FRESH_MAP = {
...
async function processGroupAWestPoint() {
  ...
  const wpRows = rows.filter(r => (r.Brand || '').toLowerCase().includes('westpoint'));
  ...
  for (let idx = 0; idx < wpRows.length; idx++) {
    ...
    if (isOk && !usedImages.has(currentUrl)) {
      ...
      // Step 2 & 3: Try to find fresh URL from official portal www.westpoint.pk
      let freshUrl = OFFICIAL_WESTPOINT_EXACT_FRESH_MAP[modelClean] || '';
  ...
  console.log("\n==================================================");
  console.log("📊 GROUP A — BRAND 3 (WESTPOINT) REPORT");
...
processGroupAWestPoint().catch(console.error);

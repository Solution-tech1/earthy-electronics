...
async function processQistAcChunk1() {
  console.log("==================================================");
  console.log("🚀 QISTBAZAAR.PK — AIR CONDITIONERS (43 MODELS) EXECUTION");
  console.log("📍 CATEGORY URL: https://www.qistbazaar.pk/product-category/air-conditioner");
  ...
  const qistAcPages = [
    'https://www.qistbazaar.pk/product-category/air-conditioner/',
    'https://www.qistbazaar.pk/product-category/air-conditioner/page/2/',
    'https://www.qistbazaar.pk/product-category/air-conditioner/page/3/',
    'https://www.qistbazaar.pk/product-category/air-conditioner/page/4/',
    'https://www.qistbazaar.pk/product-category/air-conditioner/page/5/'
  ];

  const scrapedCatalog = [];

  for (const cUrl of qistAcPages) {
    ...
    try {
      ...
      const pageItems = await page.evaluate(() => {
        ...
        links.forEach(el => {
          ...
          if (txt.length > 5 && !txt.includes('Filter') && !txt.includes('Category') && !txt.includes('Select options')) {
  ...
  console.log(`\n✅ TOTAL UNIQUE AC PRODUCTS SCRAPED FROM QISTBAZAAR: ${cleanCatalog.length}`);
  ...
  for (let idx = 0; idx < allAcItems.length; idx++) {
    ...
    if (matchedItem && matchedItem.imgSrc) {
      ...
      try {
        const tempRaw = path.join(imagesOutputDir, `temp_qist_ac_${slug}.jpg`);
  ...
  // Save report JSON
  fs.writeFileSync(path.join(__dirname, 'qist_ac_chunk1_report.json'), JSON.stringify(reportList, null, 2), 'utf8');

  console.log("\n==================================================");
  console.log("📊 QISTBAZAAR.PK — AIR CONDITIONERS CHUNK 1 EXECUTION COMPLETE");
  console.log("==================================================");
...
processQistAcChunk1().catch(console.error);

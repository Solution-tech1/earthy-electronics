...
async function restore201() {
  console.log("==================================================");
  console.log("🔄 RESTORING ALL 201 ORIGINAL PRODUCTS TO DATABASE");
  console.log("==================================================");
  ...
  for (const r of rows) {
    ...
    // Check if local file exists, else use original URL
    const slug = `${brand}-${name}`.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
    const localRel = `/images/products/${slug}.jpg`;
    const localAbs = path.join(__dirname, '..', 'frontend', 'public', 'images', 'products', `${slug}.jpg`);

    let finalImg = (r.Image_URL || r.image || '').trim();
...
restore201().catch(console.error);

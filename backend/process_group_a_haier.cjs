...
// Verified exact CDN mapping on official Haier portal (www.haier.com/pk)
const OFFICIAL_HAIER_EXACT_FRESH_MAP = {
  'hw105b14959s8': 'https://www.haier.com/pk/media/catalog/product/h/w/hw105-b14959s8.png',
  'hwm120asmw': 'https://www.haier.com/pk/media/catalog/product/h/w/hwm-120as-grey.png',
  ...
  'hw90bp14959s6': 'https://www.haier.com/pk/media/catalog/product/h/w/hw-90-bp14959-s6.png',
  'hwm1201678es9': 'https://www.haier.com/pk/media/catalog/product/h/w/hwm-120-1678.png',
...
async function processGroupAHaier() {
  ...
  const haierRows = rows.filter(r => (r.Brand || '').toLowerCase().includes('haier'));
  ...
  for (let idx = 0; idx < haierRows.length; idx++) {
    ...
    if (isOk && !usedImages.has(currentUrl)) {
      ...
      // Step 2 & 3: Try to find fresh URL from official portal www.haier.com/pk
      let freshUrl = OFFICIAL_HAIER_EXACT_FRESH_MAP[modelClean] || '';
  ...
  console.log("\n==================================================");
  console.log("📊 GROUP A — BRAND 2 (HAIER) REPORT");
...
processGroupAHaier().catch(console.error);

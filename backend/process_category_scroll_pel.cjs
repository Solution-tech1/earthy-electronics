...
async function processCategoryScrollPel() {
  console.log("==================================================");
  console.log("🚀 STARTING DIRECT CATEGORY SCROLL PROCESSOR — BRAND 4 (PEL)");
  ...
  console.log(`Loaded ${pelRows.length} PEL target products...`);
  ...
  // Direct Category URLs on pel.com.pk
  const categoryUrls = [
  ...
  for (let idx = 0; idx < pelRows.length; idx++) {
    ...
    if (match && match.imgSrc) {
      ...
      try {
        ...
        console.log(`✅ MATCHED & ATTACHED [#${idx+1}]: [PEL] ${rawModel} -> ${relativeUrl}`);
      } catch (err) {
        unverifiedCount++;
      ...
      unverifiedCount++;
      stillUnverifiedEntries.push(`"${stillUnverifiedEntries.length + 1}","PEL","${r.Category || ''}","${rawModel.replace(/"/g, '""')}","${r.SKU || ''}","${r.Rate || ''}","","STILL_UNVERIFIED","Model not listed in official category page scroll"`);
      console.log(`❌ STILL_UNVERIFIED [#${idx+1}]: [PEL] ${rawModel} (Not listed in category page scroll)`);
    }
  }
  ...
  console.log("\n==================================================");
  console.log("📊 CATEGORY SCROLL REPORT — BRAND 4 (PEL - 2 PRODUCTS)");
...
processCategoryScrollPel().catch(console.error);

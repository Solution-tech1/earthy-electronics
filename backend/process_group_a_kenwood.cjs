...
// Verified exact CDN mapping on official Kenwood portal (www.kenwoodpakistan.pk)
const OFFICIAL_KENWOOD_EXACT_FRESH_MAP = {
...
async function processGroupAKenwood() {
  ...
  for (let idx = 0; idx < kenwoodRows.length; idx++) {
    ...
    if (isOk && !usedImages.has(currentUrl)) {
      ...
      // Step 2 & 3: Try to find fresh URL from official portal www.kenwoodpakistan.pk
      let freshUrl = OFFICIAL_KENWOOD_EXACT_FRESH_MAP[modelClean] || '';
  ...
  console.log("\n==================================================");
  console.log("📊 GROUP A — BRAND 4 (KENWOOD) REPORT");
...
processGroupAKenwood().catch(console.error);

...
async function processCategoryScrollWestPoint() {
  console.log("==================================================");
  console.log("🚀 STARTING DIRECT CATEGORY SCROLL PROCESSOR — BRAND 2 (WESTPOINT)");
  ...
  console.log(`Loaded ${wpRows.length} WestPoint target products...`);
  ...
  // Direct Category URLs on westpoint.pk
  const categoryUrls = [
  ...
  for (const catUrl of categoryUrls) {
    ...
    try {
      ...
      const items = await page.evaluate(() => {
        const results = [];
        const cards = document.querySelectorAll('.product-item, .product-card, .item, li.product-item');
        cards.forEach(c => {
  ...
  for (let idx = 0; idx < wpRows.length; idx++) {
    ...
    const match = scrapedCatalog.find(item => {
      ...
      return cleanTargetDigits.length >= 3 && cleanItemTitle.includes(cleanTargetDigits);
    });
    ...
    if (match && match.imgSrc) {
      ...
      try {
        ...
        console.log(`✅ MATCHED & ATTACHED [#${idx+1}]: [WestPoint] ${rawModel} -> ${relativeUrl}`);
      } catch (err) {
        unverifiedCount++;
      ...
      unverifiedCount++;
      stillUnverifiedEntries.push(`"${stillUnverifiedEntries.length + 1}","WestPoint","${r.Category || ''}","${rawModel.replace(/"/g, '""')}","${r.SKU || ''}","${r.Rate || ''}","","STILL_UNVERIFIED","Model not listed in official category page scroll"`);
      console.log(`❌ STILL_UNVERIFIED [#${idx+1}]: [WestPoint] ${rawModel} (Not listed in category page scroll)`);
    }
  }
  ...
  console.log("\n==================================================");
  console.log("📊 CATEGORY SCROLL REPORT — BRAND 2 (WESTPOINT - 19 PRODUCTS)");
...
processCategoryScrollWestPoint().catch(console.error);
